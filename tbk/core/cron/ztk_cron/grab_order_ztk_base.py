...
__all__ = ["GrabOrderZtkCronBase"]


class GrabOrderZtkCronBase(object):
    """
    基础的抓取淘宝订单的类
    """
    ...
    def _get_order_list(
        ...
        s = self._fmt_time(start_time)
        e = self._fmt_time(end_time)
        args = NewOrderArgs(start_time=s, end_time=e, query_type=query_type)
        return self._do_get_order_list(args, logger)

    @staticmethod
    def _do_get_order_list(
        args: NewOrderArgs, logger: BoundLogger
    ) -> Optional[OrderDetailsResp]:
